...
const AddressEditor = ({ setView, onSave }) => {
  ...
  const handleSearch = async (val) => {
    ...
    try {
        const res = await fetch(`${API_URL}/geocode/search?q=${encodeURIComponent(val)}`);
  ...
  const reverseGeocode = async (lat, lon) => {
    try {
        const res = await fetch(`${API_URL}/geocode/reverse?lat=${lat}&lon=${lon}`);
        const data = await res.json();
        if (data.success) {
            setQuery(data.data.address);
            setFormattedAddress(data.data.address);
        }
    } catch (e) { console.error(e); }
  };
  ...
  return (
    // Updated container classes: added md:max-w-md md:mx-auto ... to constrain layout
    <div className="fixed inset-0 bg-white z-50 flex flex-col animate-in slide-in-from-right md:max-w-md md:mx-auto md:left-1/2 md:-translate-x-1/2 md:shadow-2xl">
      {/* Header */}
      <div className="flex items-center gap-3 p-4 border-b border-gray-100 bg-white z-10">
      ...
      <div className="p-4 bg-white relative z-20">
        <div className="relative">
            ...
            {suggestions.length > 0 && (
                <div className="absolute top-full left-0 right-0 bg-white shadow-xl rounded-xl mt-1 max-h-60 overflow-auto border border-gray-100">
                    {suggestions.map((s, i) => (
      ...
      <div className="flex-1 relative bg-gray-100">
        ...
        <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-sm px-4 py-2 rounded-full shadow-md z-[400] text-xs font-bold text-gray-700 pointer-events-none">
      ...
      {/* Footer Form */}
      <div className="p-5 bg-white border-t border-gray-100 rounded-t-3xl shadow-[0_-5px_20px_rgba(0,0,0,0.05)] relative z-20">
        <div className="mb-4">
