...
import { Clock, CheckCircle, XCircle, ShoppingBag, Bike, Download, FileText, Printer } from 'lucide-react';
          ...
          {orders.map((order) => (
            <div key={`${order.type}-${order.id}`} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between print:border-b print:shadow-none print:rounded-none">
              ...
              <div className="text-right">
                ...
                <div className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full mt-1 ${getStatusColor(order.status)} print:bg-transparent print:text-black print:p-0`}>
                  <span className="print:hidden">{getStatusIcon(order.status)}</span>
